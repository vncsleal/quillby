...
beforeAll(async () => {
  if (!fs.existsSync(SERVER_BIN)) {
    throw new Error(`Built server not found at ${SERVER_BIN}. Run 'npm run build' first.`);
  }
  ...
  runOrThrow("npm", ["run", "db:push"], env);

  const userOut = runOrThrow("npm", ["run", "keys", "create-user", "http-auth@example.com", "StrongPass123", "HTTP Auth"], env);
  const userMatch = userOut.match(/"id"\s*:\s*"([^"]+)"/);
  if (!userMatch) {
    throw new Error(`Could not parse user id from output:\n${userOut}`);
  }

  const keyOut = runOrThrow("npm", ["run", "keys", "create", userMatch[1], "http-key", "60"], env);
